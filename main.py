...
async def perform_login(client, username, password, headers):
...
headers = {
    ...
    "Referer": "https://ivas.tempnum.qzz.io/login"
}
...
async def fetch_sms(client, headers, csrf_token):
    ...
    if "login" in summary_response.text.lower():
        return None
...
async def create_key(
    ...
    conn.execute("""
        ...
        (name, api_key, max_devices, expires_at, created_at, usage_count)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (name, key, max_devices, expires_at, str(datetime.now()), 0))
    conn.commit()
...
async def bendary_api(api_key: str, username: str, password: str):
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        ...
        if session_data:
            ...
            cookies, csrf_token = await perform_login(client, username, password, headers)
            if not csrf_token:
                return JSONResponse({"error": "Login failed"}, status_code=401)
